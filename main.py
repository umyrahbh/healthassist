...
app = Flask(__name__)
app.secret_key = os.environ.get("FLASK_SECRET_KEY") or "a_secure_secret_key_for_session"
            ...
            if user:
                ...
                if password_matches:
                    ...
                    if user.user_type == 'Admin':
                        return jsonify({"redirect": "/admin/dashboard", **user_data}), 200
                    else:
...
def signup():
    ...
    try:
        ...
        required_fields = ['username', 'user_name', 'email', 'phone_number', 'gender', 'birthday', 'password']
        ...
        new_user = User(
            ...
            username=data.get('username'),
            password=generate_password_hash(data.get('password')),  # Hash the password for security
...
def get_users():
    ...
    try:
        ...
        return jsonify({"error": "Failed to retrieve users. Database connection error."}), 500
    finally:
        session.close()
...
def create_user():
    ...
    try:
        ...
        new_user = User(
            user_name=data.get('user_name'),
            gender=data.get('gender'),
            birthday=date.today(),  # For demo, using today's date
            phone_number=1234567890,  # For demo, using a placeholder
            email=data.get('email'),
            username=data.get('username'),
            password=generate_password_hash(data.get('password', 'password123')),  # Hash the password
...
def get_appointments():
    ...
    try:
        ...
        appointments = session.query(
            Appointment, User.user_name, CheckupType.name, CheckupType.price
        ).join(
            User, Appointment.user_id == User.user_id
        ).join(
            CheckupType, Appointment.checkup_id == CheckupType.checkup_id
        ).all()
        
        return jsonify([{
            'appointment_id': appt.Appointment.appointment_id,
            'user_id': appt.Appointment.user_id,
            'user_name': appt.user_name,
            'checkup_id': appt.Appointment.checkup_id,
            'checkup_name': appt.name,
            'appointment_date': appt.Appointment.appointment_date.isoformat(),
            'appointment_time': appt.Appointment.appointment_time.isoformat(),
            'price_paid': float(appt.Appointment.price_paid),
            'status': appt.Appointment.status,
            'created_at': appt.Appointment.created_at.isoformat() if appt.Appointment.created_at else None
        } for appt in appointments])
    except Exception as e:
        print(f"Error getting appointments: {str(e)}")
        return jsonify({"error": f"Failed to retrieve appointments. Error: {str(e)}"}), 500
    finally:
        session.close()
...
def get_appointment(appointment_id):
    ...
    try:
        ...
        return jsonify({
            ...
            'user_name': user_name,
            'checkup_id': appointment.checkup_id,
            'checkup_name': checkup_name,
            'appointment_date': appointment.appointment_date.isoformat(),
            'appointment_time': appointment.appointment_time.isoformat(),
            'status': appointment.status,
            'price_paid': float(appointment.price_paid),
            'created_at': appointment.created_at.isoformat() if appointment.created_at else None
        })
    except Exception as e:
        print(f"Error getting appointment: {str(e)}")
        return jsonify({"error": f"Failed to retrieve appointment. Error: {str(e)}"}), 500
    finally:
        session.close()
...
def create_appointment():
    ...
    try:
        # Validate required fields
        required_fields = ['user_id', 'checkup_id', 'appointment_date', 'appointment_time']
        for field in required_fields:
        ...
        slot_count = session.query(func.count(Appointment.appointment_id)).filter(
        ...
        new_appointment = Appointment(
            ...
            checkup_name=checkup.name,  # Add the checkup name from the checkup object
        ...
        return jsonify({
            ...
            'user_name': user.user_name,
            'checkup_id': new_appointment.checkup_id,
            'checkup_name': checkup.name,
            ...
            'price_paid': float(new_appointment.price_paid),
        ...
        if 'appointment_date' in data:
            try:
                ...
                if appointment_date != appointment.appointment_date and not is_admin:
                    ...
                    time_str = appointment.appointment_time.strftime("%H:%M:%S") if 'appointment_time' not in data else data['appointment_time']
                    time_obj = appointment.appointment_time if 'appointment_time' not in data else datetime.strptime(time_str, "%H:%M:%S").time()
                    ...
                    if slot_count >= checkup.max_slots_per_time:
                        return jsonify({"error": "The requested time slot is already fully booked"}), 400
        ...
        if 'appointment_time' in data:
            try:
                ...
                if appointment_time != appointment.appointment_time and not is_admin:
                    # Check if the new slot is available
                    date_str = appointment.appointment_date.isoformat() if 'appointment_date' not in data else data['appointment_date']
                    date_obj = appointment.appointment_date if 'appointment_date' not in data else datetime.strptime(date_str, "%Y-%m-%d").date()
...
def get_user_appointments(user_id):
    ...
    if not is_admin and not is_owner:
        return jsonify({"error": "You do not have permission to view these appointments"}), 403
    
    session = Session()
    try:
        # Join with CheckupType to get more details
        appointments = session.query(
            Appointment, CheckupType.name, CheckupType.price
        ).join(
            CheckupType, Appointment.checkup_id == CheckupType.checkup_id
        ).filter(
        ...
        return jsonify({"error": f"Failed to retrieve user appointments. Error: {str(e)}"}), 500
    finally:
        session.close()
    ...
    try:
        ...
        try:
            ...
            slot_count = session.query(func.count(Appointment.appointment_id)).filter(
                ...
                Appointment.appointment_time == datetime.strptime(appointment_time, "%H:%M:%S").time(),
                Appointment.checkup_id == int(checkup_id)
            ).scalar()
            
            if slot_count >= checkup.max_slots_per_time:
                return jsonify({'error': 'This time slot is already fully booked. Please choose another time.'}), 400
                ...
                line_items=[
                    {
                        'price_data': {
                            'currency': 'myr',
                            'product_data': {
                                'name': f'{checkup.name} Appointment',
                                'description': f'Appointment on {appointment_date} at {appointment_time}',
                            },
                            'unit_amount': int(float(checkup.price) * 100),  # Convert price to cents
                        },
                        'quantity': 1,
                    },
                ],
...
def payment_success():
    ...
    try:
        ...
        if checkout_session.payment_status != 'paid':
            return render_template('payment_error.html', error='Payment was not completed')
        ...
        try:
            ...
            # Parse date and time
            appointment_date = datetime.strptime(metadata['appointment_date'], "%Y-%m-%d").date()
            appointment_time = datetime.strptime(metadata['appointment_time'], "%H:%M:%S").time()
            ...
            if not checkup:
                return render_template('payment_error.html', error='Invalid checkup type')
                
            # Check if slot is still available (possible race condition)
            slot_count = db_session.query(func.count(Appointment.appointment_id)).filter(
                Appointment.appointment_date == appointment_date,
                Appointment.appointment_time == appointment_time,
                Appointment.checkup_id == checkup_id
            ).scalar()
            
            if slot_count >= checkup.max_slots_per_time:
                return render_template('payment_error.html', error='This time slot is now fully booked. Please contact support.')
            ...
            return render_template('payment_error.html', error=f'Error saving appointment: {str(e)}')
        finally:
            db_session.close()
            
    except Exception as e:
        print(f"Error processing payment success: {str(e)}")
        return render_template('payment_error.html', error=f'Error processing payment: {str(e)}')
...
def get_checkup_types():
    ...
    try:
        ...
        return jsonify({"error": f"Failed to retrieve checkup types. Error: {str(e)}"}), 500
    finally:
        session.close()
...
def get_checkup_type(checkup_id):
    ...
    try:
        ...
        return jsonify({"error": f"Failed to retrieve checkup type. Error: {str(e)}"}), 500
    finally:
        session.close()
...
def create_checkup_type():
    ...
    try:
        ...
        existing_checkup = session.query(CheckupType).filter(CheckupType.name == data.get('name')).first()
        if existing_checkup:
            return jsonify({"error": "Checkup type with this name already exists"}), 400
        ...
        if 'image' in request.files:
            ...
            if file and file.filename:
                ...
                upload_path = os.path.join('uploads', 'checkup_types', unique_filename)
...
def update_checkup_type(checkup_id):
    ...
    try:
        ...
        if 'name' in data and data['name'] != checkup.name:
            existing_checkup = session.query(CheckupType).filter(CheckupType.name == data['name']).first()
            if existing_checkup:
                return jsonify({"error": "Checkup type with this name already exists"}), 400
        ...
        if 'image' in request.files:
            ...
            if file and file.filename:
                ...
                # Save the file
                file.save(upload_path)
                checkup.image_path = upload_path
...
def delete_checkup_type(checkup_id):
    ...
    try:
        ...
        appointments = session.query(Appointment).filter(Appointment.checkup_id == checkup_id).first()
        if appointments:
            return jsonify({"error": "Cannot delete checkup type that is being used in appointments"}), 400
...
def get_specialists():
    ...
    try:
        ...
        return jsonify({"error": f"Failed to retrieve specialists. Error: {str(e)}"}), 500
    finally:
        session.close()
...
def get_specialist(specialist_id):
    ...
    try:
        ...
        return jsonify({"error": f"Failed to retrieve specialist. Error: {str(e)}"}), 500
    finally:
        session.close()
    ...
    try:
        ...
        if 'image' in request.files:
            ...
            if file and file.filename:
                ...
                ext = filename.rsplit('.', 1)[1].lower() if '.' in filename else ''
                unique_filename = f"{uuid.uuid4().hex}.{ext}"
                upload_path = os.path.join('uploads', 'specialists', unique_filename)
...
def update_specialist(specialist_id):
    ...
    try:
        ...
        if 'image' in request.files:
            ...
            if file and file.filename:
                # If there's an existing image, we could delete it here
                if specialist.image_path and os.path.exists(specialist.image_path):
                    try:
                ...
                # Save the file
                file.save(upload_path)
                specialist.image_path = upload_path
        elif 'image_path' in data:
...
def get_health_facts():
    ...
    try:
        ...
        return jsonify({"error": f"Failed to retrieve health facts. Error: {str(e)}"}), 500
    finally:
        session.close()
...
def get_health_fact(fact_id):
    ...
    try:
        ...
        return jsonify({
            ...
            'created_at': health_fact.created_at.isoformat() if health_fact.created_at else None
        })
    except Exception as e:
        print(f"Error getting health fact: {str(e)}")
        return jsonify({"error": f"Failed to retrieve health fact. Error: {str(e)}"}), 500
    finally:
        session.close()
...
def delete_health_fact(fact_id):
    ...
    try:
        ...
        return jsonify({
            'message': 'Health fact deleted successfully'
        })
...
def check_slot_availability():
    ...
    try:
        ...
        return jsonify({"error": f"Failed to check slot availability. Error: {str(e)}"}), 500
    finally:
        session.close()
...
def initialize_default_data():
    ...
    try:
        ...
        checkup_count = session.query(func.count(CheckupType.checkup_id)).scalar()
        if checkup_count == 0:
            # Create default checkup types
            default_checkups = [
                CheckupType(name='General Health Checkup', description='Comprehensive health assessment', price=99.00, duration_minutes=30, max_slots_per_time=10),
                CheckupType(name='Dental Checkup', description='Dental examination and cleaning', price=129.00, duration_minutes=45, max_slots_per_time=8),
                CheckupType(name='Eye Checkup', description='Vision test and eye examination', price=89.00, duration_minutes=30, max_slots_per_time=10),
                CheckupType(name='Postnatal Care', description='Care for mothers after childbirth', price=149.00, duration_minutes=60, max_slots_per_time=5),
                CheckupType(name='Prenatal Checkup', description='Care during pregnancy', price=149.00, duration_minutes=45, max_slots_per_time=8),
                CheckupType(name='Immunization', description='Vaccines and immunizations', price=69.00, duration_minutes=15, max_slots_per_time=15),
                CheckupType(name='X-Ray Diagnosis', description='X-ray imaging and diagnosis', price=199.00, duration_minutes=30, max_slots_per_time=6)
            ]
        ...
        specialist_count = session.query(func.count(Specialist.specialist_id)).scalar()
        if specialist_count == 0:
            # Create default specialists
            default_specialists = [
                Specialist(name='Dr. Sarah Johnson', title='Chief Medical Officer', specialization='General Medicine', bio='Dr. Johnson has over 15 years of experience in general medicine.'),
                Specialist(name='Dr. Michael Chen', title='Senior Dentist', specialization='Dentistry', bio='Dr. Chen specializes in cosmetic dentistry and oral health.'),
                Specialist(name='Dr. Emily Rodriguez', title='Ophthalmologist', specialization='Eye Care', bio='Dr. Rodriguez is a leading expert in diagnosing and treating eye conditions.'),
                Specialist(name='Dr. David Kim', title='OB/GYN Specialist', specialization='Women\'s Health', bio='Dr. Kim provides comprehensive care for women\'s health issues.'),
                Specialist(name='Dr. Lisa Patel', title='Pediatrician', specialization='Children\'s Health', bio='Dr. Patel has dedicated her career to providing the best care for children.')
            ]
        ...
        if fact_count == 0:
            ...
            default_facts = [
                HealthFact(title='Stay Hydrated', content='Drinking enough water is essential for your health. Aim for 8 glasses a day.', category='General Health', is_featured=1),
                HealthFact(title='Exercise Regularly', content='Regular physical activity improves cardiovascular health and mood.', category='Fitness', is_featured=1),
                HealthFact(title='Dental Hygiene', content='Brush your teeth twice a day and floss daily to prevent cavities.', category='Dental Health', is_featured=0),
                HealthFact(title='Eye Health', content='Take breaks when using digital devices to reduce eye strain.', category='Eye Care', is_featured=0),
                HealthFact(title='Prenatal Vitamins', content='Taking prenatal vitamins during pregnancy provides essential nutrients for your baby\'s development.', category='Pregnancy', is_featured=1)
            ]
